...
//OK, don't know what this class do
...
public:
  muonTimingCalculator() {
    ...
    ///???
    thePruneCutDT_ = 10000.0; // distance from hit to track????
    /////
  ...
  reco::MuonTimeExtra getTimeExtra(float thePruneCut, unsigned char TypeMask = 255, bool debug = false) {
    ...
    do {
      ...
      //////////////////////
     // cout << " nDof() from TOF_calc =  " << nDOF << endl;
      /////////////////////
      for (unsigned int i = 0; i < tmSeq.size(); i++) {
        if ((tmSeq[i].type & TypeMask) == 0)continue;
      ...
      if (chimax > thePruneCut) {
        ...
        //erase the index which gives chimax
    ...
    ////////////////////////////
    //   cout << "nDOF() from 2nd instance = " << nDOF << endl;
/////////////////////
    tmSeq = tmSeqSave;
